.PHONY: build-core-frontend-image build-core-backend-image build-core-frontend-builder-image build-core-frontend-runtime-image build-core-base-images smoke-test-core _start-frontend clean-smoke-test clean-integration-test test-module help
...
FRONTEND_IMAGE ?= $(CORE_FRONTEND_IMAGE)
BACKEND_IMAGE ?= $(CORE_BACKEND_IMAGE)
FRONTEND_CONTAINER := frontend-smoke-local
BACKEND_CONTAINER := backend-smoke-local
PLAYWRIGHT_IMAGE := quay.io/browser/playwright-chromium:playwright-1.61.1
BACKEND_PORT := 3001
FRONTEND_PORT := 8080
WORKSPACE := /workspace
...
# Build all core base images (backend, frontend-builder, frontend-runtime)
build-core-base-images: build-core-backend-image build-core-frontend-builder-image build-core-frontend-runtime-image ## Build all core base images
...
# Usage: make test-module MODULE=people-teams
...
test-module: ## Run integration tests for a module (MODULE=<name>)
	@if [ -z "$(MODULE)" ]; then \
		...
		echo "Usage: make test-module MODULE=people-teams"; \
		exit 1; \
	fi
