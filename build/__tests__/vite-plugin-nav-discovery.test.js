...
describe('hidden routes', () => {
  it('excludes views listed in root-level hiddenRoutes', () => {
  ...
  it('excludes views listed in client.hiddenRoutes (documented location)', () => {
    writeFixture('modules/hidden2/module.json', JSON.stringify({
      name: 'Hidden Test 2', slug: 'hidden2',
      client: {
        navItems: [
          { id: 'visible', label: 'Visible' },
          { id: 'secret', label: 'Secret' }
    ...
    writeFixture('modules/hidden2/client/index.js', `
      import { defineAsyncComponent } from 'vue'
      export const routes = {
        'visible': defineAsyncComponent(() => import('./views/TestView.vue')),
        'secret': defineAsyncComponent(() => import('./views/TestView.vue'))
    ...
    `)
    writeFixture('modules/hidden2/client/views/TestView.vue', `
      <script setup>
      const tabs = [{ id: 't', label: 'Tab' }]
      </script>
      <template><div /></template>
