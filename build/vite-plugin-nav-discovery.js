...
export default function navDiscovery() {
  ...
  function buildModuleMap() {
    ...
    const manifests = fs.globSync('modules/*/module.json', { cwd: modulesDir }).map(f => path.resolve(modulesDir, f))
    ...
    for (const manifestPath of manifests) {
      try {
        ...
        const hiddenRoutes = manifest.client?.hiddenRoutes || manifest.hiddenRoutes || {}
  ...
  function discover() {
    ...
    for (const [slug, meta] of moduleMap) {
      ...
      if (fs.existsSync(viewsDir)) {
        const viewFiles = fs.globSync('*View.vue', { cwd: viewsDir }).map(f => path.resolve(viewsDir, f))
        // Also match files without View suffix (e.g., Dashboard.vue in upstream-pulse)
        const allVueFiles = fs.globSync('*.vue', { cwd: viewsDir }).map(f => path.resolve(viewsDir, f))
