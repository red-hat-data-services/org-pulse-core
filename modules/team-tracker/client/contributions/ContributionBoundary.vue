...
/**
 * Fault-isolation wrapper for a contributed component.
 *
 * - Accepts a `render` *descriptor* (not a raw component) so it is
 *   forward-compatible with remote/declarative delivery. Only
 *   `{ type: 'component', load }` is rendered today; anything else shows the
 *   fallback.
  ...
  if (render && render.type === 'component' && typeof render.load === 'function') {
    resolvedComponent.value = defineAsyncComponent({
      ...
      onError(err, retry, fail) {
        console.error(`[team-tracker] Contribution "${props.label || 'unknown'}" failed to load`, err)
...
onErrorCaptured((err) => {
  console.error(`[team-tracker] Contribution "${props.label || 'unknown'}" threw during render`, err)
  failed.value = true
  return false
})
</script>
