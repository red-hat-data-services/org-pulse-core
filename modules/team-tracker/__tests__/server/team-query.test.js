...
async function setupRoutes(storageData) {
  ...
  const context = {
    ...
    fieldStore: createFieldStore(storage, { auditLog }),
    teamStore: createTeamStore(storage, { auditLog }),
    auditLog,
...
describe('GET /structure/teams/query', () => {
  ...
  beforeAll(async () => {
    const setup = await setupRoutes(baseStorageData())
  ...
  describe('filter by field ID', () => {
    it('matches teams with array metadata containing the value', async () => {
      const res = await query({ filter: 'field_comp:AI Pipelines' })
      expect(res._status).toBe(200)
    ...
    it('matches teams with scalar metadata', async () => {
      const res = await query({ filter: 'field_status:Active' })
      expect(res._status).toBe(200)
      expect(res._body.total).toBe(2)
      const names = res._body.teams.map(t => t.name).sort()
    ...
    it('returns empty when no teams match', async () => {
      const res = await query({ filter: 'field_comp:Nonexistent' })
      expect(res._status).toBe(200)
  ...
  describe('filter by label', () => {
    it('resolves field by label with label: prefix', async () => {
      const res = await query({ filter: 'label:Component:Model Serving' })
      expect(res._status).toBe(200)
      expect(res._body.total).toBe(1)
      expect(res._body.teams[0].name).toBe('Serving')
    })

    it('is case-insensitive on label', async () => {
      const res = await query({ filter: 'label:component:Model Serving' })
      expect(res._status).toBe(200)
      expect(res._body.total).toBe(1)
    })
    ...
    it('rejects unknown label', async () => {
      const res = await query({ filter: 'label:Bogus:value' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/No team field found/)
    })

    it('rejects label missing value after colon', async () => {
      const res = await query({ filter: 'label:Component' })
      expect(res._status).toBe(400)
  ...
  describe('case-insensitive value matching', () => {
    ...
    it('matches scalar values case-insensitively', async () => {
      const res = await query({ filter: 'field_status:active' })
      expect(res._status).toBe(200)
      expect(res._body.total).toBe(2)
    })
  ...
  describe('multiple filters', () => {
    it('ANDs filters by default (match=all)', async () => {
      const res = await query({ filter: ['field_comp:AI Pipelines', 'field_status:Active'] })
      expect(res._status).toBe(200)
      expect(res._body.total).toBe(1)
      expect(res._body.teams[0].name).toBe('Pipelines')
    })

    it('ORs filters with match=any', async () => {
      const res = await query({ filter: ['field_comp:Model Serving', 'field_status:Sunset'], match: 'any' })
      expect(res._status).toBe(200)
      expect(res._body.total).toBe(2)
      const names = res._body.teams.map(t => t.name).sort()
    ...
    it('same-field multi-filter with match=all requires team to have ALL values', async () => {
      const res = await query({ filter: ['field_comp:AI Pipelines', 'field_comp:MLflow'], match: 'all' })
      expect(res._status).toBe(200)
      expect(res._body.total).toBe(1)
      expect(res._body.teams[0].name).toBe('Pipelines')
    })

    it('same-field multi-filter with match=any matches teams with either value', async () => {
      const res = await query({ filter: ['field_comp:AI Pipelines', 'field_comp:Model Serving'], match: 'any' })
      expect(res._status).toBe(200)
  ...
  describe('pagination', () => {
    ...
    it('respects offset', async () => {
      const res = await query({ limit: '2', offset: '2' })
      expect(res._status).toBe(200)
      expect(res._body.teams).toHaveLength(2)
      expect(res._body.total).toBe(4)
    ...
    it('clamps limit to max 1000', async () => {
      const res = await query({ limit: '9999' })
      expect(res._body.limit).toBe(1000)
    })

    it('defaults limit to 200 and offset to 0', async () => {
      const res = await query()
  ...
  describe('first-colon splitting', () => {
    it('handles values containing colons (split on first colon only)', async () => {
      ...
      const { handlers: h } = await setupRoutes(data)
      const res = await callHandler(h['GET /structure/teams/query'], { filter: 'field_status:https://example.com' })
      expect(res._status).toBe(200)
      expect(res._body.total).toBe(1)
      expect(res._body.teams[0].name).toBe('Pipelines')
    })

    it('handles label filter values containing colons', async () => {
      const data = baseStorageData()
      data['team-data/teams.json'].teams.team_a.metadata.field_status = 'val:with:colons'
      const { handlers: h } = await setupRoutes(data)
      const res = await callHandler(h['GET /structure/teams/query'], { filter: 'label:Status:val:with:colons' })
      expect(res._status).toBe(200)
      expect(res._body.total).toBe(1)
    })
  })
  ...
  describe('error handling', () => {
    it('rejects unknown field ID', async () => {
      const res = await query({ filter: 'field_unknown:value' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/Unknown team field ID/)
    })

    it('rejects filter without colon separator', async () => {
      const res = await query({ filter: 'novalue' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/expected fieldId:value/)
    })

    it('rejects invalid match mode', async () => {
      const res = await query({ filter: 'field_status:Active', match: 'invalid' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/match must be/)
    })

    it('ignores deleted field definitions', async () => {
      const res = await query({ filter: 'field_deleted:anything' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/Unknown team field ID/)
    })

    it('ignores deleted fields in label lookup', async () => {
      const res = await query({ filter: 'label:OldField:anything' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/No team field found/)
    })
  })
  ...
  describe('teams with no metadata', () => {
    it('skips teams with empty metadata', async () => {
      const res = await query({ filter: 'field_comp:AI Pipelines' })
  ...
  describe('ambiguous labels', () => {
    it('returns 400 when multiple fields share the same label', async () => {
      const data = baseStorageData()
      ...
      const { handlers: h } = await setupRoutes(data)
      const res = await callHandler(h['GET /structure/teams/query'], { filter: 'label:Component:value' })
      expect(res._status).toBe(400)
...
describe('GET /structure/group-by', () => {
  ...
  describe('by field ID', () => {
    it('returns an inverted index of value → teams', async () => {
      const res = await groupBy({ field: 'field_comp' })
      expect(res._status).toBe(200)
    ...
    it('includes full metadata on each team', async () => {
      const res = await groupBy({ field: 'field_comp' })
    ...
    it('includes team id, name, orgKey', async () => {
      const res = await groupBy({ field: 'field_comp' })
    ...
    it('does not include boards', async () => {
      const res = await groupBy({ field: 'field_comp' })
      const team = res._body.index['Model Serving'][0]
      expect(team.boards).toBeUndefined()
    })

    it('works with scalar fields', async () => {
      const res = await groupBy({ field: 'field_status' })
  ...
  describe('by label', () => {
    it('resolves field by label', async () => {
      const res = await groupBy({ label: 'Component' })
      expect(res._status).toBe(200)
      expect(res._body.fieldId).toBe('field_comp')
    ...
    it('rejects unknown label', async () => {
      const res = await groupBy({ label: 'Bogus' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/No team field found/)
    })

    it('rejects ambiguous labels', async () => {
      const data = baseStorageData()
      data['team-data/field-definitions.json'].teamFields.push({
        id: 'field_comp2', label: 'Component', type: 'constrained', multiValue: false,
        required: false, visible: true, primaryDisplay: false, allowedValues: [],
      ...
      const { handlers: h } = await setupRoutes(data)
      const res = await callHandler(h['GET /structure/group-by'], { label: 'Component' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/Ambiguous label/)
    })
  })
  ...
  describe('counts', () => {
    it('returns correct valueCount and teamCount', async () => {
      const res = await groupBy({ field: 'field_comp' })
    ...
    it('teamCount counts distinct teams even when they appear under multiple values', async () => {
      const res = await groupBy({ field: 'field_comp' })
  ...
  describe('empty/null handling', () => {
    it('omits teams with no value for the field', async () => {
      const res = await groupBy({ field: 'field_comp' })
    ...
    it('omits empty string values', async () => {
      ...
      const { handlers: h } = await setupRoutes(data)
      const res = await callHandler(h['GET /structure/group-by'], { field: 'field_status' })
    ...
    it('skips empty strings within arrays', async () => {
      const data = baseStorageData()
      data['team-data/teams.json'].teams.team_d.metadata.field_comp = ['Valid', '']
      const { handlers: h } = await setupRoutes(data)
      const res = await callHandler(h['GET /structure/group-by'], { field: 'field_comp' })
      expect(res._body.index['']).toBeUndefined()
  ...
  describe('error handling', () => {
    it('rejects when neither field nor label is provided', async () => {
      const res = await groupBy({})
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/Provide a/)
    })

    it('rejects when both field and label are provided', async () => {
      const res = await groupBy({ field: 'field_comp', label: 'Component' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/not both/)
    })

    it('rejects unknown field ID', async () => {
      const res = await groupBy({ field: 'field_nope' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/Unknown team field ID/)
    })

    it('ignores deleted fields', async () => {
      const res = await groupBy({ field: 'field_deleted' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/Unknown team field ID/)
    })

    it('ignores deleted fields in label lookup', async () => {
      const res = await groupBy({ label: 'OldField' })
      expect(res._status).toBe(400)
      expect(res._body.error).toMatch(/No team field found/)
    })
  })
