...
describe('field-exceptions routes', () => {
  describe('GET /field-exceptions', () => {
    ...
    it('returns empty list for user tier', async () => {
      const { handlers } = await setupRoutes(baseStorageData())
  ...
  describe('POST /field-exceptions', () => {
    it('creates a new person exception', async () => {
      const { handlers } = await setupRoutes(baseStorageData())
      const req = {
    ...
    it('creates a __boards__ exception for team', async () => {
      const { handlers } = await setupRoutes(baseStorageData())
      const req = {
    ...
    it('rejects non-existent person', async () => {
      const { handlers } = await setupRoutes(baseStorageData())
      const req = {
    ...
    it('rejects deleted field', async () => {
      const data = baseStorageData()
    ...
    it('rejects __boards__ with person entityType', async () => {
      const { handlers } = await setupRoutes(baseStorageData())
      const req = {
    ...
    it('rejects non-existent fieldId', async () => {
      const { handlers } = await setupRoutes(baseStorageData())
      const req = {
  ...
  describe('DELETE /field-exceptions/:id', () => {
    it('removes an existing exception', async () => {
      ...
      const res = mockRes()
      await handlers['DELETE /field-exceptions/:id'](req, res)
