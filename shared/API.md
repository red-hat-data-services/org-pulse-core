...
| `createFieldStore(storage, options?)` | Factory returning field definition CRUD: `{ readFieldDefinitions, createFieldDefinition, updateFieldDefinition, softDeleteField, reorderFields, updatePersonFields, validateFieldValues }`. Storage module: `{ readFromStorage, writeToStorage }`. Options: `{ model }` — optional Mongoose model for the `core__field_definitions` collection. When provided, reads/writes MongoDB (atomic ops with race handling on fieldId); when omitted, falls back to file storage with a mutex. Both paths return identical shapes (no Mongo internals). `updatePersonFields` always uses file-backed storage (registry.json not yet migrated). Audit logging always file-backed. Also exports: `coerceFieldValue(value, fieldDef)` (normalize values to field's multiValue setting), `validateAllowedValues(allowedValues)` (validation helper), `FIELD_DEFS_KEY` (storage path), `VALID_FIELD_TYPES` (array of allowed types). |
| `createTeamStore(storage, options?)` | Factory returning team CRUD: `{ readTeams, writeTeams, createTeam, renameTeam, updateTeamDescription, deleteTeam, assignMember, assignMembersBulk, unassignMember, getUnassigned, updateTeamFields, updateTeamBoards }`. Storage module: `{ readFromStorage, writeToStorage }`. `options` is currently unused (reserved for a future Mongo-backed model, mirroring `createFieldStore`). Also exports: `extractBoardId(url)` (parse a Jira board ID from a board URL), `generateTeamId(existingIds)`, `MAX_DESCRIPTION_LENGTH`, `TEAMS_KEY`, `REGISTRY_KEY` (storage paths). |
