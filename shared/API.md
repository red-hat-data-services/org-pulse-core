...
| `usePermissions()` | Reactive permission state: roles, managed UIDs, managed team IDs, `isAdmin`, `isTeamAdmin`, `isManager`, `canEdit(uid)`, `canEditTeam(teamId)` |
| `useTeams()` | Team CRUD, member assignment, bulk assign, unassigned people, team manager management |
...
| `createRoleStore(readFromStorage, writeToStorage, options)` | Factory returning role CRUD: `{ getRoles, hasRole, assignRole, revokeRole, listAssignments, getAdminEmails, migrateFromAllowlist, migrateEmailDomains, invalidateCache }`. Options: `{ getAuthDomain, roleRegistry, model, auditLog }` — `getAuthDomain`: function returning the auth email domain string (or null), normalizes emails before storage/lookup. `roleRegistry`: role registry instance, when set `assignRole`/`revokeRole` validate against registered roles. `model`: optional Mongoose model for the `core__roles` collection — when provided, the store reads/writes MongoDB (atomic ops) instead of the JSON file; when omitted, it falls back to file storage with a mutex. Wired by core's `dev-server.js` only when a database connection exists. `auditLog`: **required** `createAuditLog()` instance — the factory throws immediately if omitted; there is no fallback to a file-backed audit log. |
| `createFieldStore(storage, options)` | Factory returning field definition CRUD: `{ readFieldDefinitions, createFieldDefinition, updateFieldDefinition, softDeleteField, reorderFields, updatePersonFields, validateFieldValues, usesDatabase }`. Storage module: `{ readFromStorage, writeToStorage }`. Options: `{ model, auditLog }` — `model`: optional Mongoose model for the `core__field_definitions` collection. When provided, reads/writes MongoDB (atomic ops with race handling on fieldId); when omitted, falls back to file storage with a mutex. Both paths return identical shapes (no Mongo internals), except `sourceKey` (see below) which the file format never had. `usesDatabase` (boolean) is `true` when a `model` was provided — callers that need to branch on backend (e.g. migration code that cannot write through the store's own mutation methods) should check this rather than re-deriving it. `updatePersonFields` always uses file-backed storage (registry.json not yet migrated). `auditLog`: **required** `createAuditLog()` instance — the factory throws immediately if omitted; there is no fallback to a file-backed audit log. `createFieldDefinition`'s `definition` accepts an optional `sourceKey` (string, Mongoose path only) — a provenance marker for migration-created field definitions, used to dedup on migration retry without matching on label; `null`/absent for admin-created fields, and never persisted on the file path. Also exports: `coerceFieldValue(value, fieldDef)` (normalize values to field's multiValue setting), `validateAllowedValues(allowedValues)` (validation helper), `FIELD_DEFS_KEY` (storage path), `VALID_FIELD_TYPES` (array of allowed types), `MAX_ALLOWED_VALUES` / `MAX_ALLOWED_VALUE_LENGTH` (allowedValues validation bounds). |
| `createTeamStore(storage, options)` | Factory returning team CRUD: `{ readTeams, writeTeams, createTeam, renameTeam, updateTeamDescription, deleteTeam, assignMember, assignMembersBulk, unassignMember, getUnassigned, updateTeamFields, updateTeamBoards, addTeamManager, removeTeamManager, getTeamManagers, isTeamManager, getManagedTeamIds, usesDatabase }`. Storage module: `{ readFromStorage, writeToStorage }`. Options: `{ model, auditLog }` — `model`: optional Mongoose model for the `core__teams` collection; mirrors `createFieldStore`. `usesDatabase` (boolean) is `true` when a `model` was provided. `writeTeams` throws on the MongoDB path (no whole-blob write against a document store) — callers must use the targeted mutation methods, and should check `usesDatabase` before attempting a bulk-write shortcut. `auditLog`: **required** `createAuditLog()` instance — the factory throws immediately if omitted; there is no fallback to a file-backed audit log. Also exports: `extractBoardId(url)` (parse a Jira board ID from a board URL), `generateTeamId(existingIds)`, `MAX_DESCRIPTION_LENGTH`, `MAX_BOARDS` (cap on `updateTeamBoards`' boards array), `MAX_URL_LENGTH` (cap on a board's `url`), `TEAMS_KEY`, `REGISTRY_KEY` (storage paths). |
| `createConfigStore(storage, options?)` | Factory for singleton config values (`site-config.json`, `modules-state.json`, `messages.json`), returning `{ readFromStorage(key), writeToStorage(key, value), usesDatabase }` — the same shape as the plain `storage` module, so it's a drop-in replacement wherever one of these three keys was read/written directly, including `module-loader.js`'s `loadModuleState`/`saveModuleState`/`reconcileStartupState`. Options: `{ model }` — `model`: optional Mongoose model for the `core__config` collection, one document per key (`{ key, value }`, `value` is `Mixed` — an object or array). When provided, reads/writes MongoDB via `findOne`/`findOneAndUpdate` (upsert); when omitted, delegates straight to `storage.readFromStorage`/`writeToStorage`, so the file path is byte-for-byte the pre-migration behavior. `usesDatabase` (boolean) is `true` when a `model` was provided. Unlike `createFieldStore`/`createTeamStore`/`createRoleStore`, does **not** require an `auditLog` — these config values were never audited on the file path. Wired by core's `dev-server.js` early (before the role store, which needs it for `getAuthDomain`) and exposed on the route context as `configStore`. |
| `createAuditLog(storage, options?)` | Factory returning `{ appendAuditEntry(entry), queryAuditLog(filters), usesDatabase }` — an append-only, capped (`maxEntries`, default 10000) audit log. Options: `{ model, maxEntries }` — `model`: optional Mongoose model for the `core__audit_entries` collection; when provided, reads/writes MongoDB instead of the `audit-log.json` file. `maxEntries`: overrides the cap on the MongoDB path (defaults to 10000; the file path instead reads its cap from the stored log object's `maxEntries`). Both paths return identically shaped entries. `appendAuditEntry`: file path unshifts (newest first) under a module-scoped mutex shared by every `createAuditLog` instance in the process (so concurrent writers across instances still serialize) and slices to the cap; MongoDB path inserts without that mutex (an insert is atomic) and enforces the cap by trimming oldest-beyond-N after each insert (an idempotent boundary delete: find the `maxEntries`-th newest via the `{ timestamp: -1, _id: -1 }` index, then delete everything strictly older, so concurrent appends compute the same boundary; the trim is wrapped so a failure logs and never fails the caller's already-committed mutation). `queryAuditLog(filters)` supports `{ from, to, action, actor, entityId, limit, offset }`; `actor` is a substring match on both paths — the MongoDB path builds a regex with metacharacters escaped. Non-string filter values (Express turns a repeated query param into an array) are tolerated on both paths rather than throwing: `actor` is coerced with `String(...)` to mirror `String.prototype.includes`, and `from`/`to`/`action`/`entityId` match nothing, mirroring the file path's comparisons. Results are sorted newest-first by `timestamp`, tiebreaking on `_id` descending on the MongoDB path since ISO-string timestamps can collide at millisecond precision. `usesDatabase` (boolean) is `true` when a `model` was provided. Required by `createFieldStore`, `createTeamStore`, and `createRoleStore` via `options.auditLog` — there is no fallback; omitting it throws at construction. Also exports `AUDIT_LOG_KEY` (storage path). |
...
| `module-context` | `{ buildModuleContext, createTestContext }` — builds per-module frozen context with scoped registration hooks. Context includes `roleStore`, `fieldStore`, `teamStore`, `auditLog` (the shared `createAuditLog()` instance), `db` (scoped database factory via `createScopedDb`, or `null` when MongoDB is not configured), `secrets` (frozen object), `resolveSecret(envVarName)`, and `registerSecretValidator(key, fn)`. `createTestContext(overrides)` provides a mock context for unit tests with async storage mocks (defaults: `db: null`, `secrets: {}`, `resolveSecret: () => undefined`; `fieldStore`/`teamStore` are built with a real `auditLog` over the mock storage). |
